...
Color Scene::colorAtPoint(SPSceneObject so, const Vector3F &pIntersect) const
{
    ...
    { 
        ...
        retColor += ((*iter)->get_color()) * (so->get_surface_color()) * fmaxf(dot(N, L), 0);
    }
...
Color Scene::traceRay(const Ray &r, int depth /*= 0*/) const
{
    Color retColor(0,0,0);
    float t;
    ...
    if (so_r != 0 && depth > 0)
    {
        ...
        Color reflect_c = traceRay(r.reflect(intersect_loc, so->surface_normal(intersect_loc)), depth - 1);

        retColor = so_r * reflect_c + ((1 - so_r) * retColor);
    }
